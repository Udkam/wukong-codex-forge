...
- **双场景图库已修复为可验证状态**：新建任务从 4 张战斗图轮换，进入对话从 5 张风景图轮换；每组使用独立 session 游标，双固定层先解码目标图再以 820 ms 交叉淡变，始终 `cover` 全窗。V13 修复隐藏旧对话误判、任意侧栏点击换图、损坏游标空背景、ResizeObserver 自循环、覆盖层损坏和快速切换闪帧。
- **V13.3 延续硬资源预算**：首屏不构造额外 `Image`，不再启动解码全部 9 张背景；场景切换最多保留一个可取消的解码请求，稳态只保留一个背景纹理，820 ms 过渡期最多两个。旧层在过渡后清空，`will-change` 只在过渡期存在，全屏滤镜与无意义缩放已移除。
...
- **输入框已进入 V21 多轮验收**：主纸面按 `184:25` 比例钳制在 `96–120px`，最大原生列宽 `736px` 时约为 `736×100px`；排队消息与目标仍处于同一原生 stack，但每个原生行独立叠一层只含两个上角、直底边的纸面，新增消息不会拉长一张总背景；独立 progress pill 才是全圆。原生文字、按钮、ARIA、底部锚点和命中区保持不变，技术门禁通过不等于用户视觉验收完成。
