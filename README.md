...
> 当前开发线为 **0.12.3 / V13.3**。V13.2 及更早实现继续原样保留为可审计历史；与现行行为冲突时，以 `docs/CURRENT_GOAL.md` 及本节的 V13.3 合同为准。

## V13 当前实现与审计状态

- **活动范围是全窗背景 + 新建页题字/图案**：顶部栏、侧栏、环境信息、对话、composer、按钮、尺寸、命中区和交互保持原生，回答保持无框。当前 landing 审稿候选只在原 56×56 图标位内绘制“横置金箍棒 + 三道墨尾”，并在原题字位显示“此去，欲破何局？”，不新增卡片、栏位、按钮或 emoji；用户确认前不视为最终定稿。
...
- **双场景图库已修复为可验证状态**：新建任务从 6 张战斗图轮换，进入对话从 5 张风景图轮换；每组使用独立 session 游标，双固定层先解码目标图再以 820 ms 交叉淡变，始终 `cover` 全窗。V13 修复隐藏旧对话误判、任意侧栏点击换图、损坏游标空背景、ResizeObserver 自循环、覆盖层损坏和快速切换闪帧。
- **V13.3 延续硬资源预算**：首屏不构造额外 `Image`，不再启动解码全部 11 张背景；场景切换最多保留一个可取消的解码请求，稳态只保留一个背景纹理，820 ms 过渡期最多两个。旧层在过渡后清空，`will-change` 只在过渡期存在，全屏滤镜与无意义缩放已移除。
...
- **56×56 图案已重绘**：撤销残缺金箍，改为两端带赤金箍纹的金箍棒与三道墨尾；原生图案占位、尺寸、布局和热区均不变。
- **输入框仍在独立审稿**：V1–V7 均已否决并原样保留。V8 提供“残卷墨界 / 石印绳契 / 丹炉铜契”三个原生 `736×96`、零外扩、零控件位移的预览，但尚未获得用户选择，活动 runtime 不加载任何候选 composer 样式。
...
- **Hatch Pet 分开验收**：小八戒 repair-v2 已形成独立候选；小天命人现有基础立绘仍缺神锋后棍身/尾端和右脚厌火魔足，base 与派生图集继续冻结，不进入新发行物。用户新录制 `Replay 2026-07-24 00-30-17.mkv` 只作为跑动与背面棍花的动作参考；背面视角不足以推断正面握法、面部或衣甲。
...
V13.3 背景/landing 五项定向浏览器场景均通过：除 `cover` 全窗、主表面透明、DOMRect 不变、稳态单纹理和单请求解码外，还覆盖“标题与图案延迟挂载且父级仍为 opacity 0”的真实首帧条件，并证明不触发 resize 也会完成替换。真实 Codex 独立调试包仍只在截图和资源采样期间临时启动，取证后立即关闭；输入框和宠物继续以视觉审查为发布门槛。
