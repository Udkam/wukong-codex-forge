...
- **输入框已进入 V20 多轮验收**：主纸面按 `184:25` 比例钳制在 `96–120px`，最大原生列宽 `736px` 时约为 `736×100px`；joined queue/goal stack 只绘制两个上角，独立 progress pill 才是全圆。原生文字、按钮、ARIA、底部锚点和命中区保持不变，技术门禁通过不等于用户视觉验收完成。
