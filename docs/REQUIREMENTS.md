...
> **V34 现行发布合同。** 下表延续 V15 的视觉/几何编号；V13.3 及更早内容作为历史保留，冲突处以 `CURRENT_GOAL.md`、V15-32 至 V15-36 和本表为准。
...
V34 当前证据：保留式 release `0.13.0-20260801-144611` 已安装，两个受管开始菜单入口均指向 Codex 内置 Node 与同一 append-only bridge，仓库/安装 host 哈希一致；生命周期与恢复定向合同 23/23、最小包合同 1/1。剩余验收只包含一个绿色资源窗口下的真实完整页 queue/goal 视觉与截图后精确资源归零，不包含两只延期宠物或已取消葫芦。
