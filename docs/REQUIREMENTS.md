...
| V12-04 | composer 候选不得进入 runtime，直到用户审稿通过；V1–V3 继续保留为失败历史 | 注入资源清单与 runtime style 审计 |
| V12-05 | composer 不改变 `736×96` / `560×96`、原控件坐标、提示词和命中区；必须在 1× 下无需标题即可识别夜叉衣甲、金箍棒和神锋 | 1×/4×审稿、12 态几何与盲审 |
