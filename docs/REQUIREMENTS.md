...
| V15-05 | 新建页保留原生 56×56 槽位；卡通短棍与墨尾贴纸已否决，当前候选必须由真实金箍棒模型生成，保留深红棍身、兽纹端箍和旧金层次，不得像 emoji、贴纸或通用短棍 | 112×112 透明资产像素/体积合同、1× landing 局部审查、槽位 DOMRect/原节点/restore 合同与用户实机验收 |
...
| V15-14 | 活动背景与 UI 位图除压缩字节上限外必须有解码像素硬门：单背景 ≤12,000,000 px、图库唯一文件总计 ≤32,000,000 px、任意两张最大背景合计 ≤16,000,000 px、单 UI/装饰位图 ≤4,194,304 px；尺寸头无效或超限时必须在 payload 组装前失败 | JPEG/PNG/WebP 头解析、实际 11 图与 8 UI 资产统计、内存内 100,000×100,000 PNG 拒绝测试、最小包导入测试 |
...
V15-05 当前技术候选使用真实金箍棒模型生成 112×112 / 3,218-byte 透明 WebP；在原生 56×56 槽位的有效边界约为 50×42 px。四边透明、居中差、主轴角度、9%–13% 可见面积、体积、无 CSS 几何覆盖和最小包门禁已通过；最新无头证据为 `artifacts/test-runs/v15-native-surfaces-2026-07-25T04-49-40-653Z/08-landing-mark-56.png`，仍待用户实机视觉验收。

V15-14 当前实测：11 张唯一背景共 22,220,472 px，最大两张合计 4,743,680 px；8 张活动 UI WebP 单张最大 580,608 px。`great-sage-return.jpg` 的 1256×707 仍低于 1080p 画质要求，只是资源安全通过，不是视觉验收通过。
