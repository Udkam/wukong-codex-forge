...
V38 当前证据：保留式 release `0.13.0-20260803-143153` 已安装，两个受管开始菜单入口均指向 Codex 内置 Node 与同一 append-only bridge，仓库/安装 host 哈希一致；环境卡一体纸面与来源锁定 footer fade 定向合同 4/4，生命周期与恢复合同 23/23、最小包合同 1/1。剩余验收只包含一个绿色资源窗口下的真实完整页 queue/goal 视觉与截图后精确资源归零，不包含两只延期宠物或已取消葫芦。
