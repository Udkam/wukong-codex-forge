...
V42 当前证据边界：保留式 release `0.13.0-20260803-143153` 仍是已安装基线，两个受管开始菜单入口均指向 Codex 内置 Node 与同一 append-only bridge，仓库/安装 host 哈希一致；生命周期与恢复合同 23/23、最小包合同 1/1。用户实机截图已证明 V38 环境标题夹具不完整，因此环境一体纸面必须重新通过 V15-39 定向回归、安装新 release 与绿色资源窗口下的真实完整页复验；不包含两只延期宠物或已取消葫芦。
