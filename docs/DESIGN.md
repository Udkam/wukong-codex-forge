...
八张活动 UI WebP 合计约 271 KiB，其中 landing 金箍棒只有 3.2 KiB；材质静态绘制，无 timer、动画、网络请求、常驻 filter 或持续合成提示。强制高对比模式隐藏全部材质并恢复系统面。葫芦已从当前目标删除，不进入活动主题或包。V15 当前只完成 fixture、像素与本机源码合同，不等于真实 Codex 用户验收。
