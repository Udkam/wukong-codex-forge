...
官方 `[data-testid="home-icon"]` 提供 56×56 原生槽位。旧 SVG 短棍因比例、饱和金红、圆润端头和附加墨尾呈现卡通贴纸感，已否决。V15 改用真实金箍棒模型的透明正投影：裁去透明边、在 4× 工作分辨率压到 48×7、旋转 39° 后降采样为 112×112 WebP。最终只通过 `--forge-ui-landing-mark` 在原槽位绘制，保留原始 SVG 节点、DOMRect、aria 和 restore；棍身为低饱和深红，端箍保留兽纹起伏与旧金层次，不另加徽章、阴影或动画。

### 成本与回退

八张活动 UI WebP 合计约 273 KiB，其中 landing 金箍棒只有 2.4 KiB；材质静态绘制，无 timer、动画、网络请求、常驻 filter 或持续合成提示。强制高对比模式隐藏全部材质并恢复系统面。葫芦已从当前目标删除，不进入活动主题或包。V15 当前只完成 fixture、像素与本机源码合同，不等于真实 Codex 用户验收。
