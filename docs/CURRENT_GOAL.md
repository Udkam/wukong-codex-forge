...
- 当前生命周期与恢复定向合同为 23/23、最小发布包合同为 1/1，`releasedPetIds` 为空且安装阶段对宠物无操作。当前剩余联合门是：先完成 V42 环境标题承载层回归与新 release 安装，再在绿色资源窗口用一个临时受管实例取得含侧栏、顶部栏、工作区、queue/goal、短卷输入器与环境卡的完整页实机证据，并在截图后证明 root、host、子进程与端口全部归零。
