...
- 原生编辑器、footer、上下文条、排队/目标条、项目条、按钮尺寸、文字、ARIA、动态状态和命中区继续保留；只允许编辑器外壳与 footer 在卷页内获得与参考图相符的安全内边距。排队消息与进行中目标继续共用贴在主卷页上方的原生 stack，但不得把一张纸面随条目数纵向拉长：每个原生行必须独立叠一层左上/右上切角、直底边的纸面，增加一条消息就增加一层图案；独立计划/变更进度 pill 才保留完整圆角。
...
- V17 曾把排队消息与进行中目标识别为一个官方 above-composer stack，并只在该 stack 外层绘制一次纸面；这一“总背景随条目拉长”的绘制策略已被 V21 逐行叠层合同取代。V17 的八状态证据继续保留为历史，不再代表当前绘制方式。
...
- V20 依据用户最新两张原生状态截图把主卷页收至 `clamp(96px, width × 25 / 184, 120px)`，最大原生列宽 `736px` 时精确为 `100px`；editor wrapper 恢复 12px 原生横向内距，footer 恢复原生 8px 横向内距与 8px 底距。该几何继续有效，但 V20 的 stack 外层单次绘制已被 V21 逐行绘制取代。
- V21 使用无中央透明洞的暗暖灰黄赭静态纸面，并把排队消息/进行中目标的绘制宿主下沉到每个原生行；stack 自身不再绘制背景。三行夹具证明每一行都拥有独立 `pointer-events:none` 纸面、只有两个上角且底边为直线，原生行仍连续、尺寸和按钮命中不变；独立 progress pill 仍全圆。无头证据位于 `artifacts/test-runs/v21-per-row-paper-stack-20260731/`，当前仍待用户实机视觉验收。
