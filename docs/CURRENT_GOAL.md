...
- 纸面综合色阶以用户最新暗色参考为准：当前重建目标为 `RGB(135,117,93)`，实际四类产物内区中位色约 `RGB(131–135,111–117,86–93)`；主输入器、上下文/目标条、计划/变更条和项目上下文条必须从同一透明源重建并使用同一暖灰黄赭 matte，不能分别套滤镜或出现亮黄断层。
...
- 原生编辑器、footer、上下文条、排队/目标条、项目条、按钮尺寸、文字、ARIA、动态状态和命中区继续保留；只允许编辑器外壳与 footer 在卷页内获得与参考图相符的安全内边距。排队消息与进行中目标共用一个贴在主卷页上方的原生 stack：只绘制左上、右上两个角，底边保持直线，不得出现左下、右下角饰；独立计划/变更进度 pill 才保留完整圆角。
...
- V20 依据用户最新两张原生状态截图把主卷页收至 `clamp(96px, width × 25 / 184, 120px)`，最大原生列宽 `736px` 时精确为 `100px`；editor wrapper 恢复 12px 原生横向内距，footer 恢复原生 8px 横向内距与 8px 底距。排队消息/进行中目标仍只由 joined stack 外层绘制左上、右上两个角，内部接缝和最下边均为直线；独立 progress pill 仍全圆。无头状态证据位于 `artifacts/test-runs/v20-composer-native-proportion-20260730-2/`，仍待用户实机视觉验收。
