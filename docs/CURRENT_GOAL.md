...
- 保持原生输入框尺寸、最大宽度、动态增长、布局、按钮位置和热区，只替换形状、边界、材质与细节；审稿使用当前实机 `736×98` / `560×98`，但生产实现不得锁死高度。
- 不修改用户提示词、占位文案或回答内容；助手回答保持无额外外框。
- 黑神话元素必须通过结构、材质和克制的器物细节融入，不能仅换色，也不能生硬贴图。
- V1–V8 已因泛古风套壳、器物贴片、可读性或原生几何证据失真被否决。当前 V9 的“章回残墨 / 金箍锁锋 / 大圣翎影”三个候选仅供选择，未获选择前不得写入正式运行时。
...
- 已完成输入框 V9 三案的 736/560 双宽度与 98/154 双高度审稿；主题前后 DOMRect、文字、ARIA 和五点命中区全等，且候选为零外部请求、零位图解码、零 timer/animation/filter。方案尚未获得用户选择，正式运行时未实施。
- 宠物高保真外观与动作仍待重做。
