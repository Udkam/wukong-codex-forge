...
- 主输入器不再完全套用原生高度或圆角外框：宽度与底部锚点仍由原生布局负责，外框使用 `256:63` 卷页比例，并限制在 `168–256px` 高度内；不得写死某一窗口截图的绝对宽度。
- 外形使用轻量切角卷页轮廓与静态纸面，不沿用原生圆角矩形，也不使用会把纸面中央挖空的 alpha 位图遮罩。
- 原生编辑器、footer、上下文条、排队/目标条、项目条、按钮尺寸、文字、ARIA、动态状态和命中区继续保留；只允许编辑器外壳与 footer 在卷页内获得与参考图相符的安全内边距。
...
- V16 主输入器已按用户最新例外改为受限卷页几何：宽度与底部锚点保持原生，外框按 `256:63` 在 `168–256px` 内响应；编辑器外壳和 footer 获得卷页安全内边距，按钮尺寸、文字、ARIA 与状态拓扑保持原生。六状态无头证据位于 `artifacts/test-runs/v16-expanded-composer-20260730-132359/`，当前只视为本轮技术候选，仍待用户视觉验收。
