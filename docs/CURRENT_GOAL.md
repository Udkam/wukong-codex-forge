...
- 纸面综合色阶以用户最新暗色参考为准：目标内区中位色约 `RGB(126,112,96)`；主输入器、上下文/目标条、计划/变更条和项目上下文条必须从同一透明源重建并使用同一暗色 matte，不能分别套滤镜或出现亮黄断层。
...
- 已增加图片头级解码像素门禁：单背景不超过 1200 万像素、11 图总计不超过 3200 万像素、过渡中两张最大图合计不超过 1600 万像素、单张 UI 材质不超过 419 万像素。当前图库实测总计 22,220,472 像素，最坏双图过渡 4,743,680 像素；伪造的 100,000×100,000 PNG 会在组装 payload 前被拒绝。
- `great-sage-return.jpg` 仍只有 1256×707，低于用户的 1080p 素材要求；它是已知画质缺口，不能因资源门禁通过而视为背景素材验收完成。
