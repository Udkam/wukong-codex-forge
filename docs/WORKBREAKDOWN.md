...
| 主对话 · V17 composer topology | 保持原生 DOM、状态和按钮尺寸，把新建对话主输入器实现为短卷页，并按真实 topology 区分 joined stack 与独立 pill | `184:25`、`120–168px` 已接入；真实 `736px` 新建对话约 `120px`。fixture 已撤下测试专用 component 身份并改用生产直属子节点关系；排队/目标 row 的官方上角 token 与无下角 token 已进入 9 项定向门禁，独立 progress pill 继续全圆，等待用户视觉审计 |
