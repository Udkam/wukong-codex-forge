...
- 本轮未创建子代理、浏览器服务、开发服务器或监听器；主对话独立完成只读 ASAR 核对、实现、定向测试、文档和精确发布。
- 实现域：composer 首帧结构门禁、queue/goal 原生黑边中和、环境 Section/标题映射。
- 验证域：侧栏选中态、透明首帧目标、guided queue/goal、300px 环境卡和 forced-colors 五个独立测试，不运行全量套件。
- 下一域：安装当前提交后仅启动一个临时受管 Codex 调试窗口，取得包含侧栏、顶部栏、工作区、环境卡、目标/排队与输入器的完整页截图，随后立即关闭并核验资源归零。

## 2026-08-03 V38 当前分工与验收边界

- 本检查点不创建子代理、开发服务器、浏览器服务或常驻监听；主对话只处理环境卡连续纸面、原生 thread footer 渐隐绘制和定向证据。
- 实现域：环境主标题与三个分区标题改为透明承载，让卡片唯一纸面贯穿全部内容；只清除 `data-thread-scroll-footer` 内层、无交互的渐隐 paint，保留原生 sticky 布局、尺寸、障碍层和命中区。
- 验证域：侧栏仅选中态、Framer Motion 首帧目标、300px 环境卡与原生 footer fade 四项定向回归；环境夹具同步锁定 7 行、3 分区、3 分区标题和完整页截图，不运行全量套件。
- 证据域：`artifacts/test-runs/v38-environment-unified-20260803-141204/01-full-multi-guided.png` 是 `1600×900` 原生结构夹具整页证据，不是生产 Codex 截图，也不据此标记真实联合门完成。
- 资源域：一次临时真实窗口尝试触发 CPU 100% 后立即中止，确认原生恢复并精确释放所属进程与端口；后续不得为补截图重新打开该便携 profile。

## 2026-08-03 V39 文档真值收口

- 本检查点只校正现行 README、需求、设计、目标、分工与工作日志；不改运行时、素材、宠物策略、安装脚本或本地安装状态。
- 把 V11/V10 的宠物与葫芦说明明确降级为历史记录，现行合同继续以“宠物延期、葫芦取消”为准。
- 把安装证据更新到 append-only release `0.13.0-20260803-143153`，并把 V38 环境卡一体纸面、7 行/3 分区与来源锁定 footer fade 纳入现行验收合同。
- 验证只运行文档真值搜索、Markdown 路径检查与 Git 精确 diff；不启动浏览器、调试窗口或全量测试。

## 2026-08-03 V40 真实整页取证门稳定化
...
## 2026-08-03 V42 环境标题承载层补强

- 实现域：主标题不再使用单次 `closest()`；只沿已定位标题到官方外卡的祖先链透明化标题/header 与显式 dropdown-background 承载层。三个分区继续使用当前 ASAR 的 Section/直属 header 双签名，并同时撤下基础层及双伪元素 paint。
- 几何域：不改 300px 外卡、标题、Section、7 行、折叠、滚动、加号、链接、ARIA、DOM 顺序或命中区；唯一纸面仍由外卡绘制。
- 验证域：夹具先证明标题承载层与伪元素在注入前确实为深色，再证明注入后主标题链、三个分区标题的基础层与双伪元素透明，且所有 DOMRect 和加号命中前后全等。用户实机截图已否决 V38 视觉结论，V42 未经真实完整页复验不得标记视觉完成。
- 资源域：CPU 处于琥珀线时只运行语法、diff 和文档检查；Playwright、正式安装和唯一临时 Codex 完整页取证等待绿色窗口。
