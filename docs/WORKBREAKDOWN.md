...
4. M4：安装到真实 `CODEX_HOME`，由用户审计实际 Codex 页面颜色。结果：失败，运行中窗口无变化。
5. M5：核对主进程加载路径与深链，确认外部配置无外观热应用入口。
6. M6：撤销假成功表述，定向测试、文档、commit 与 push。
7. M7：等待用户决定是否允许一次窗口重载/应用重启，或重新授权非原生注入路线。
