...
test('public install and disable entry points preserve every existing file', () => {
  ...
  assert.match(hook, /runtime\\host\.mjs/);
  ...
  assert.doesNotMatch(hook, /EncodedCommand|WindowsPowerShell|Get-CimInstance/);
  assert.match(hook, /const themeAvailable = fs\.existsSync\(marker\) && fs\.existsSync\(host\)/);
