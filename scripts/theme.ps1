...
$importPath=[IO.Path]::GetFullPath($Import);if(-not (Test-Path -LiteralPath $importPath)){throw 'Theme JSON was not found.'}
$imagePath=$null;if($Image){$imagePath=[IO.Path]::GetFullPath($Image);if(-not (Test-Path -LiteralPath $imagePath)){throw 'Background image was not found.'}}
Push-Location $rootPath;try{node runtime/forge-runtime.mjs --validate $importPath;if($LASTEXITCODE){throw 'Theme validation failed.'}
$theme=Get-Content -LiteralPath $importPath -Raw -Encoding utf8|ConvertFrom-Json
